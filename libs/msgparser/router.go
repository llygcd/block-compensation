...
func (rtr *router) AddRoute(path string, h Handler) Router {
	...
	if !IsAlphaNumeric(path) {
		logrus.Error("addroute failed for route expressions can only contain alphanumeric characters")
		return rtr
	}
	if rtr.HasRoute(path) {
		logrus.Error(fmt.Sprintf("route %s has already been initialized", path))
		return rtr
	}
